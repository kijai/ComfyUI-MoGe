...
import numpy as np
from ...utils3d.numpy import image_pixel_center, image_uv, sliding_window_2d
...
def mask_aware_nearest_resize_numpy(mask: np.ndarray, target_width: int, target_height: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    uv = image_pixel_center(width=width, height=height, dtype=np.float32)
    ...
    windowed_mask = sliding_window_2d(padded_mask, (filter_h_i, filter_w_i), 1, axis=(-2, -1))
    ...
    target_uv = image_uv(width=target_width, height=target_height, dtype=np.float32) * np.array([width, height], dtype=np.float32)
